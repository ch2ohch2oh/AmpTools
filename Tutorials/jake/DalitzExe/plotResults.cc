...
int main( int argc, char* argv[] ){
  ...
  //
  AmpToolsInterface::registerAmplitude(AngularBreitWigner());
  AmpToolsInterface::registerAmplitude(LambdaKPiAngles());
  AmpToolsInterface::registerAmplitude(Xi0KKAngles());
  AmpToolsInterface::registerAmplitude(Xi0phiHelicity());  
  ...
  for (unsigned int ivar  = 0; ivar  < DalitzPlotGenerator::kNumHists; ivar++){
    ...
    Histogram* hist = plotGenerator.projection(ivar,
            ...
            string xtitle = "Mass(#Xi^{0}K^{+}) (GeV/c^{2})";
    if (ivar == 1) xtitle = "Mass(#Xi^{0}K^{-}) (GeV/c^{2})";
    if (ivar == 2) xtitle = "Mass(K^{+}K^{-}) (GeV/c^{2})";
